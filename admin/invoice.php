<?php
	...
	$qSql.= "LEFT JOIN tblcustomers ON tblpayments.cus_id = tblcustomers.ID ";
...
<html>
	...
	<body>
		<header>
			<h1>Invoice</h1>
			<address>
				<p>Chic Beauty Salon</p>
				<p>Lagro, Quezon City, Metro Manila</p>
				<p>+93123 4567890</p>
			</address>
		...
		<article style="position:relative">
			...
			<address>
				<p><?= $invInfo['Name'] ?><br><?= $invInfo['MobileNumber'] ?></p>
			</address>
			<table class="meta">
				<tr>
			...
			<table class="inventory">
				...
				<?php
					$total = 0;
					foreach(explode(",", $invInfo['services']) as $servId) {
						$sInfo = $con->query("SELECT * FROM tblservices WHERE ID = '{$servId}'")->fetch_assoc();
						$total += $sInfo['Cost'];
				?>
					<tr>
						<td><?= $sInfo['ServiceName'] ?></span></td>
						<td><span>PHP</span><span><?= $sInfo['Cost'] ?></span></td>
					</tr>
				<?php } ?>
				</tbody>
			</table>
			<table class="balance">
				<tr>
					...
					<td><span>PHP</span><span><?= $total ?></span></td>
				</tr>
				<tr>
					<th><span>Amount Paid</span></th>
					<td><span>PHP</span><span><?= $invInfo['paid_amount'] ?></span></td>
				</tr>
				<tr>
					<th><span>Balance Due</span></th>
					<td><span>PHP</span><span><?= $total-$invInfo['paid_amount'] ?></span></td>
				</tr>
			</table>
