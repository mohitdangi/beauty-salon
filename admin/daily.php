...
<html>
<head>
<title>Chic Beauty Salon |  Sales Reports</title>
					...
					<div class="form-grids row widget-shadow" data-example-id="basic-forms"> 
						...
						<div class="form-body">
							<table class="table table-bordered">
								...
								<tbody>
								<?php
									$ret = mysqli_query($con, "SELECT * FROM tblappointment GROUP BY AptDate ");
									$total_tcost = $total_paid = 0;
									$sl = 1;
									while($row=mysqli_fetch_array($ret)) {
										$tCost = 0;
										$fDate = date("Y-m-d", strtotime($row['AptDate']));
										$a_inDay = $con->query("SELECT * FROM tblappointment WHERE AptDate = '{$row['AptDate']}'");
								...
								<tfoot>
									<tr>
										...
										<td></td>
										<td></td>
										<td>PHP<?php echo $total_tcost ;?></td>
										<td>PHP<?php echo $total_paid ;?></td>
									</tr>
								</tfoot>
							</table>
						</div>
